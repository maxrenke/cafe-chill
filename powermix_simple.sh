...
# Simple bash script to run powermix_direct.py
# This version saves files directly to /DATA/Media/Music/C895/Powermix
# Track numbering is now handled within the Python script

# Get the directory where this script is located
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PYTHON_SCRIPT="$SCRIPT_DIR/powermix_direct.py"

# Check if Python script exists
if [ ! -f "$PYTHON_SCRIPT" ]; then
    echo "Error: powermix_direct.py not found in $SCRIPT_DIR"
    exit 1
fi
...
echo "Running powermix_direct.py..."
